...
def login_page():	
	...
	try:
		if request.method=="POST":#the action part of form gives this feed
			...
			#flash(password)
			c,conn=connection()
			notempty=c.execute("SELECT * FROM users")
			
			if notempty :	
				
				flash("welcome")				
				return redirect(url_for('dash'))#it redirects to the url of dash
			else:
...
def currentlyfree(name):
	return "hello "+name+", umm.. you are not registered and for now it doesn't matter. So enjoy."#here 'name' is a variable.
